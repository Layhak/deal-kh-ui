...
//   currentPage, category , discountType
// }) => 
//  {
...
//   const [discountData, setDiscountData] = useState<CartProductType[]>([]);
//   const [totalPages, setTotalPages] = useState<number>(1);
  
//   useEffect(() => {
//     if (data?.payload) {
//       setDiscountData(data.payload.list);
//       setTotalPages(data.payload.pagination.totalPages);
//     }
//   }, [data]);

//   const handleProductClick = (slug: string) => {
//     // Example of using a Link component from Next.js for navigation
//     router.push(`/products/${slug}`);
//   };

//   const handlePageChange = (page: number) => {
//     // Update current page state to trigger re-fetching of products
...
const DiscountCardComponent: React.FC<DiscountCardComponentProps> = ({
  ...
  useEffect(() => {
    if (data?.payload) {
      setDiscountData(data.payload.list);
      const totalProducts = data.payload.pagination.total; // Ensure your API returns total number of products
      const totalPages = Math.ceil(totalProducts / 2); // Assuming page size is 2
  ...
  return (
    <main>
      {/* Card section */}
      <div className="flex flex-wrap justify-center gap-[25px]">
        {discountData.map((product: CartProductType) => (
          <Card
            ...
            <CardBody className="relative h-[260px] overflow-visible rounded-b-lg px-4">
              <Link href={`/products/${product.slug}`}>
                <Image
                  className="h-[193px] w-[284px] object-cover"
                  isZoomed
                  src={product.images[0]?.url || 'https://example.com/default-image.jpg'}
                  alt={product.name}
                />
              </Link>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex">
                    {Array.from({ length: 5 }, (_, index) => (
                      <StarIcon
                        key={index}
                        filled={product.ratingAvg >= index + 1}
                        half={product.ratingAvg >= index + 0.5 && product.ratingAvg < index + 1}
                        className="h-4 w-4 text-yellow-500"
                      />
                    ))}
                    <span className="ml-1 text-[13px] font-medium text-foreground-600">
              ...
              <Link href={`/products/${product.slug}`}>
                <h5 className="mt-1 h-[45px] text-[18px] font-semibold tracking-tight text-foreground-800">
                  {product.name.length > 60 ? `${product.name.substring(0, 45)}...` : product.name}
                </h5> 
              </Link>
              <div className="h-[30px] pt-2">
                <p className="text-[14px] font-medium text-foreground-600">
