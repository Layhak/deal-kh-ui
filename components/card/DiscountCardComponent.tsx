...
const DiscountCardComponent: React.FC<DiscountCardComponentProps> = ({ currentPage }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`/api/discounts?page=${currentPage}&limit=6`);
        const result = await response.json();
        setDiscountData(result.data); // Ensure this matches your data structure
      } catch (error) {
              ...
              <div className="h-[30px] pt-2">
                <p className="text-[14px] font-medium text-foreground-600">
                  Shop :{' '}
                    ...
                    <Link className='lowercase' href={`/shop/slug/${product.shop.toLowerCase().replaceAll(' ','-')}`}>
...
// const DiscountCardComponent = () => {
//   const [currentPage, setCurrentPage] = useState(1);
//   const pageSize = 10; // Assuming 10 items per page

//   const handlePageChange = (page: number) => {
//     setCurrentPage(page);
//     // You may fetch data for the new page here, e.g., using Redux or fetching from an API
//   };

//   const totalItems = 50; // Total number of items (you would typically get this from API or state)

//   // Logic to get the current page's items
//   const startIndex = (currentPage - 1) * pageSize;
//   const endIndex = Math.min(startIndex + pageSize, totalItems);
//   const currentItems = Array.from({ length: endIndex - startIndex }, (_, index) => ({
...
//       {currentItems && (
//         <div className="flex flex-wrap justify-between gap-[25px]">
//           {currentItems.list.map((product: CartProductType) => (
//             <Card
//               key={product.slug}
//               onClick={() => router.push(`products/${product.slug}`)}
//               isPressable
...
//                       classNames={{
//                         base: 'bg-gradient-to-br from-indigo-500 to-pink-500 border-small border-white/50 shadow-pink-500/30',
//                         content: 'drop-shadow shadow-black text-gray-100 text-small',
//                       }}
//                       size={'sm'}
//                     >
//                       {product?.category ?? 'New'}
...
//                 <Link href={`products/${product.slug}`}>
//                   <h5 className="mt-1 h-[45px] text-[18px] font-semibold tracking-tight text-foreground-800">
//                     {product.name.length > 60 ? `${product.name.substring(0, 45)}...` : product.name || 'Product Name'}
//                   </h5>
//                 </Link>
//                 <div className="h-[30px] pt-2">
//                   <p className="text-[14px] font-medium text-foreground-600">
...
//                     <Link className='lowercase' href={`/shop/slug/${product.shop.toLowerCase().replaceAll(' ', '-')}`}>
//                       <span className="text-info-800 text-[14px] font-medium">
//                         {product.shop.length > 30 ? `${product.shop.substring(0, 20)}...` : product.shop || 'Shop Name'}
//                       </span>
//                     </Link>
//                   </p>
//                   <p className="text-[14px] font-medium text-foreground-600">
...
// import { CartProductType } from '@/libs/difinition';
// import { useGetProductsQuery } from '@/redux/service/product';
// import { Card, CardBody, Chip, Image, Link, Pagination } from '@nextui-org/react';
// import { useRouter } from 'next/navigation';
...
//   const { data, isLoading, error } = useGetProductsQuery({
//     page: 1,
//     size: 1000, // Fetch all products at once
//     field: '',
//     fieldName: '',
//   });

//   const items = data?.payload?.list || [];
...
//     <main>
//       <div className="flex flex-wrap justify-between gap-[25px]">
//         {currentItems.map((product: CartProductType) => (
//           <Card
//             onClick={() => router.push(`products/${product.slug}`)}
//             key={product.slug}
//             isPressable
