...
import { useCreateShopRatingMutation, useCreateShopReportMutation, useGetShopReportBySlugQuery } from '@/redux/service/shopReportAndRating';
...
const validationSchema = Yup.object().shape({
  ...
  shopSlug: Yup.string().required('Product slug is required'),
  images: Yup.array().of(
    Yup.object().shape({
      file: fileValidation,
    })
...
export default function ReviewFormShop({
  shopSlug,
  onNewRating,
  hasRated,
}: ReviewFormProps) {
  const [uploadImages] = useUploadImagesMutation();
  const [createFeedback] = useCreateShopReportMutation();
  const [createRating] = useCreateShopRatingMutation();
  const productSlug = shopSlug;
            ...
            <div className="absolute right-0 flex h-full flex-col items-end justify-between gap-2">
              ...
              <div className="flex items-end gap-2">
                ...
                {isLoadingUserData ? (
                  ...
                  <Button
                    color={
                      !values.description || hasRated
                        ? 'default'
                        : 'primary'
                    }
                    isDisabled={!values.description || hasRated}
                    radius="lg"
                    size="sm"
                    variant="solid"
                    className={
                    ...
                    onPress={openModal}
                  >
                    {hasRated ? 'You Already Reported' : 'Send message'}
                  </Button>
                </Tooltip>
