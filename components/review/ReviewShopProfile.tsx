...
} from '@/redux/service/shopReportAndRating';
...
export default function ReviewShopProfile({
  ...
  } = useGetShopRatingByShopSlugQuery({ shopSlug: productSlug });
  const {
    data: feedbackData,
    isLoading: feedbackLoading,
    refetch: refetchFeedback,
  } = useGetShopReportBySlugQuery({ shopSlug: productSlug });

  useEffect(() => {
    if (ratingsData && currentUserProfile) {
      const userHasRated = ratingsData?.payload.some(
        (rating: any) =>
          rating.username === currentUserProfile?.payload?.username
      );
